...
TrilatEKF::TrilatEKF(VectorXd xInit, MatrixXd sensorLoc) {
    ...
    MatrixXd PInit = MatrixXd::Zero(STATE_SIZE, STATE_SIZE);            // initial state is not completely known
    PInit(2,2) = 10;
    PInit(3,3) = 10;
    std::cout << "xInit: " << xInit << std::endl;
    MatrixXd HInit = getJacobian(sensorLoc, xInit);     // calculate initial measurement matrix H
    
    // initialize Kalman Filter with xInit
...
void TrilatEKF::processMeasurements(const std::vector<TrilatMeasurement> &measurements){
    ...
    ekf_.updateMahalanobis(distanceMatrix);

    std::cout << "x updated: " << ekf_.x_(0) << ", " << ekf_.x_(1) << std::endl;
}
...
vector<TrilatMeasurement> TrilatEKF::getCombinations(vector<Measurement> mVec){
    ...
    tmVec[4] = toTrilatMeasurement(mVec[1], mVec[2], mVec[4]);
    tmVec[5] = toTrilatMeasurement(mVec[1], mVec[2], mVec[5]);
    tmVec[6] = toTrilatMeasurement(mVec[1], mVec[3], mVec[4]);
    tmVec[7] = toTrilatMeasurement(mVec[1], mVec[3], mVec[5]);
    return tmVec;
}
